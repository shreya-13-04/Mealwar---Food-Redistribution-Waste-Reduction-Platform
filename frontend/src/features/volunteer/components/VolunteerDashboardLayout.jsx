...
const VolunteerDashboardLayout = () => {
    ...
    return (
        <div className="volunteer-dashboard" style={{ display: 'flex', minHeight: '100vh', backgroundColor: 'var(--bg-light)' }}>
            ...
            <aside className={mobileMenuOpen ? 'mobile-open' : ''} style={{ width: '260px', backgroundColor: 'var(--white)', borderRight: '1px solid #eee', display: 'flex', flexDirection: 'column', padding: '2rem 1.5rem' }}>
                <div style={{ marginBottom: '2.5rem', cursor: 'pointer' }} onClick={() => navigate('/')}>
                    ...
                    <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Volunteer Panel</p>
                </div>

                <nav style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', flex: 1 }}>
                    <DashboardLink to="/volunteer/dashboard" end>Home</DashboardLink>
                    <DashboardLink to="/volunteer/dashboard/orders">My Deliveries</DashboardLink>
                    <DashboardLink to="/volunteer/dashboard/verification">Verification</DashboardLink>
                    <DashboardLink to="/volunteer/dashboard/ratings">Ratings & Badges</DashboardLink>
                    <DashboardLink to="/volunteer/dashboard/profile">My Profile</DashboardLink>
                </nav>
            ...
            <main style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                <header style={{ height: '70px', backgroundColor: 'var(--white)', borderBottom: '1px solid #eee', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 2rem' }}>
                    <h3 style={{ margin: 0 }}>Hi, {user?.name || 'Volunteer'}</h3>
                    <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
                        ...
                        <span
                            onClick={() => navigate('/volunteer/dashboard/notifications')}
                            style={{ cursor: 'pointer', position: 'relative' }}
                        >
                            🔔
                            <span style={{ position: 'absolute', top: '-5px', right: '-5px', backgroundColor: 'var(--secondary-color)', color: 'white', fontSize: '10px', padding: '2px 5px', borderRadius: '10px' }}>2</span>
                        </span>
                        <div style={{ position: 'relative' }}>
                            ...
                            {showProfileMenu && (
                                <div style={{ position: 'absolute', right: 0, top: '45px', backgroundColor: 'white', boxShadow: '0 4px 12px rgba(0,0,0,0.15)', borderRadius: '8px', minWidth: '150px', zIndex: 1000 }}>
                                    <div
                                        onClick={() => { navigate('/volunteer/dashboard/profile'); setShowProfileMenu(false); }}
