...
const BuyerDashboardLayout = () => {
    ...
    return (
        <div className="buyer-dashboard" style={{ display: 'flex', minHeight: '100vh', backgroundColor: 'var(--bg-light)' }}>
            ...
            <aside className={mobileMenuOpen ? 'mobile-open' : ''} style={{ width: '260px', backgroundColor: 'var(--white)', borderRight: '1px solid #eee', display: 'flex', flexDirection: 'column', padding: '2rem 1.5rem' }}>
                <div style={{ marginBottom: '2.5rem', cursor: 'pointer' }} onClick={() => navigate('/')}>
                    <h2 style={{ color: 'var(--secondary-color)', margin: 0 }}>MealWar</h2>
                    <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Buyer Panel</p>
                </div>

                <nav style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', flex: 1 }}>
                    <DashboardLink to="/buyer/dashboard" end>Home</DashboardLink>
                    <DashboardLink to="/buyer/dashboard/search">Find Food</DashboardLink>
                    <DashboardLink to="/buyer/dashboard/orders">My Orders</DashboardLink>
                    <DashboardLink to="/buyer/dashboard/favourites">Favourites</DashboardLink>
                    <DashboardLink to="/buyer/dashboard/profile">My Profile</DashboardLink>
                </nav>
            ...
            <main style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                <header style={{ height: '70px', backgroundColor: 'var(--white)', borderBottom: '1px solid #eee', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 2rem' }}>
                    <h3 style={{ margin: 0 }}>Hi, {user?.name || 'Friend'}</h3>
                    <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
                        ...
                        >
                            🔔
                            <span style={{ position: 'absolute', top: '-5px', right: '-5px', backgroundColor: 'var(--secondary-color)', color: 'white', fontSize: '10px', padding: '2px 5px', borderRadius: '10px' }}>0</span>
                        </span>
                        <div style={{ position: 'relative' }}>
                            ...
                            {showProfileMenu && (
                                <div style={{ position: 'absolute', right: 0, top: '45px', backgroundColor: 'white', boxShadow: '0 4px 12px rgba(0,0,0,0.15)', borderRadius: '8px', minWidth: '150px', zIndex: 1000 }}>
                                    <div
                                        onClick={() => { navigate('/buyer/dashboard/profile'); setShowProfileMenu(false); }}
